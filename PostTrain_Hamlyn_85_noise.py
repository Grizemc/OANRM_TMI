...
def post_train_main(model, args, io, sample_num, data, loss_fn):
    ...
    for epoch in range(0, args.pt_epoch):
        ...
        if epoch == 0:
            ...
            string = "supervise test_acc is {}, mask_acc is {}, precise_pred_mask_acc is {}".format(best_result1[0], best_result1[1],
                                                                                      best_result1[2])
            io.cprint(string)
    ...
    # 每个样本循环完毕后测精度
        string = "unsupervised test_acc is {}, mask_acc is {}, precise_pred_mask_acc is {}".format(best_result[0], best_result[1],
                                                                                  best_result[2])
        io.cprint(string)
...
if __name__ == "__main__":
    ...
    args_g, io_g = SH_PostTrainInit("Post_Train_Hamlyn_no_rotation_75_datiao_qiuqiu_0624test4_rotation")
    ...
    test_hamlyn_loader = DataLoader(HamlynArtificialFPFH(
        root="/big_data/szm/H8amlyn_8192_Mask_3332_new_mutual_Low_Overlap/test",
        fpfh_path="/big_data/szm/H8amlyn_8192_Mask_3332_new_mutual_Low_Overlap/fpft_file",
        sample_num=sample_num, normalize=False, continuous=True),
        batch_size=1)
    # test_hamlyn_loader = DataLoader(HamlynArtificialFPFH(
    #     root="/big_data/szm/H8amlyn_8192_Mask_3332_new_mutual_85noise/test_7.0_0.40",
    #     fpfh_path="/big_data/szm/H8amlyn_8192_Mask_3332_new_mutual_85noise/fpft_file_7.0_0.40",
    #     sample_num=sample_num, normalize=False, continuous=True),
    #     batch_size=1)
    # test_hamlyn_loader = DataLoader(HamlynArtificialFPFH(
