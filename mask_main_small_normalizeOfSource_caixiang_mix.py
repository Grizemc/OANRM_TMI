...
def init():
    ...
    parser.add_argument('--config', type=str, default='/home/szm/Paconv_730/config/Source_Flow_softmax_topkpoint_topmask_fuse_8192_mix.yaml',
                        help='config file')
...
def main(args, io, writer):
    ...
    if args.resume is not None:
        ...
        resume_filename = os.path.join("./checkpoints", args.exp_name, "saved_model",
                                       "train_epoch_{}_end.t7".format(args.resume))
        io.cprint("Resuming From {}".format(resume_filename))
        # checkpoint = torch.load(resume_filename)
        starting_epoch = 44  # checkpoint['epoch'] + 1
        model.load_state_dict(torch.load(resume_filename))
