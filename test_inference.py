...
parser.add_argument('--pretrained-model', dest='pretrained_model', required=False,
                        default='sceneflow_pretrained.tar', metavar='PATH', help='path to pre-trained SSDNet model')
parser.add_argument('--calib-path', dest='calib_path', required=False,
                        default=r'D:\try\try\MIOL\calib_1105.yaml', metavar='PATH', help='path to stereo calibration file')
...
parser.add_argument("--dataset-dir", required=False, default=r"D:\try\try\MIOL\output_frames\rect_right_gai", type=str, help="Dataset directory")
parser.add_argument("--output-depth", action='store_true', help="save depth img")
parser.add_argument("--output-dir", required=False, default= 'D://try//try//MIOL//output_frames_1105_dep2//',type=str, help="Output directory")
...
def cal_LK(left_img_rgb, right_img_rgb, intrinsics, scale, downsample):
    ...
    st = st.reshape(int(h/downsample), int(w/downsample)).astype(np.bool_)
...
def inference(args, left_img_input, right_img_input, debug_transform, intrinsics, pose, pose_inv, disp_net, optimizer, record_idx):
    ...
    # data to GPU
    left_img = imgs[0].unsqueeze(0).to(device)
    right_img = imgs[1].unsqueeze(0).to(device)
    intrinsics = [torch.from_numpy(intrinsic).unsqueeze(0).to(device) for intrinsic in intrinsics]
    pose = torch.from_numpy(pose).unsqueeze(0).to(device)
    pose_inv = torch.from_numpy(pose_inv).unsqueeze(0).to(device)
    ...
    if args.output_depth:
        ...
        # save img-depth map
        # 二维数组vis_tgt_depth，存储深度信息
        vis_tgt_depth = left_depth.detach().cpu().numpy()[0,0]

        vis_pred = depth_visualizer(vis_tgt_depth).astype(np.uint8)
        vis_lm = lm[0].cpu().numpy().repeat(3,0).transpose(1,2,0).astype(np.bool_)

        # 将数组vis_pred中在~vis_lm中对应位置为True
        vis_pred[~vis_lm] = 0

        hh, ww, _ = left_img_input.shape
        sum_img = np.zeros((hh, ww*3, 3))
        ...
        dep_path = os.path.join(args.output_dir, "show_dep{:06}.png".format(record_idx))
        ...
        cv2.imwrite(dep_path,vis_tgt_depth)
...
def main():
    ...
    # right_imgs = [file.replace('left', 'right') for file in left_imgs]

    for j in tqdm(range(len(left_imgs))):
        
        left = cv2.imread(os.path.join(args.dataset_dir, left_imgs[j]))
        right = cv2.imread(os.path.join(r"output_frames/rect_left_gai", left_imgs[j]))
