...
from util.data import HuaXiFpfh,StereoFpfh
...
def post_train_main(model, args, io, index, data, loss_fn):
    ...
    # model_path = r"/home/szm/Paconv/checkpoints/Source_Flow_softmax_topkpoint_topmask_fuse_8192/saved_model/best_model.t7"
    ...
    for epoch in range(0, args.pt_epoch):
        ...
        if type(sample_num) == type(1):
            ...
            if not os.path.exists(
                    rf"/home/szm/Paconv_730/duibi_keshihua_stereo_noise/P2-5_192-377/result/{noise_scale}"):
                os.makedirs(rf"/home/szm/Paconv_730/duibi_keshihua_stereo_noise/P2-5_192-377/result/{noise_scale}")
            save_path = f"/home/szm/Paconv_730/duibi_keshihua_stereo_noise/P2-5_192-377/result/{noise_scale}/post_train_epoch_{epoch}.npz"
            np.savez(save_path,
                     points2=points2.cpu(),
                     points1=points1.cpu(),
                     colors1=colors1.cpu(),
...
if __name__ == "__main__":
    # 用stereomis数据集，sample_num要比自己选的图片减去700
    sample_num = 192 # 192  # 670 100
    mask_truncation = 0.9
    g_type_name = 'Stereo_0610_P2-5Fpfh{}_{}'.format(sample_num,377)
    noise_scale = "z0.5_c0.06"  # z1.0_c0.06 z0.5_c0.06
    
    # train_loader = DataLoader(
    #     StereoFpfh(root=r"/big_data/szm/cwz_stereomis_downsample_0.5", sample_num=sample_num,
    #               fpfh=True, normalize=False, interval=377), batch_size=1)
    sample_num = 0
    train_loader = DataLoader(
        StereoFpfh(root=rf"/home/szm/Paconv_730/duibi_keshihua_stereo_noise/P2-5_192-377/noise/{noise_scale}", sample_num=sample_num,
    ...
    if type(sample_num) == type(1):
        ...
        for index, data in loop:
            ...
            post_train_main(model_g, args_g, io_g, index, data, loss_fn)
        end_time = datetime.datetime.now()
        if not os.path.exists(rf"/home/szm/Paconv_730/duibi_keshihua_stereo_noise/P2-5_192-377/result/{noise_scale}"):
            os.makedirs(rf"/home/szm/Paconv_730/duibi_keshihua_stereo_noise/P2-5_192-377/result/{noise_scale}")
        save_path = f"/home/szm/Paconv_730/duibi_keshihua_stereo_noise/P2-5_192-377/result/{noise_scale}/post_train_epoch_{epoch}.npz"
