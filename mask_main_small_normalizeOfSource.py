...
# 有监督训练的文件，用的M8ICCAI_8192_Mask_19055_new_mutual
# 训练集13873 测试集2677
...
def init():
    ...
    parser.add_argument('--config', type=str, default='/home/cwz/paconv_/config/Source_Flow_softmax_topkpoint_topmask_fuse_8192.yaml',
                        help='config file')
    ...
    if args.train:
        ...
        # io 对象用于记录详细的文本信息，适合于日志文件。
        # writer 对象用于记录和可视化更结构化的信息，比如损失值、模型权重的直方图等，适合于 TensorBoard 或其他类似工具的可视化输出。
        # util.py文件中有IOS
        ...
        # pythonfile = "/Source_Flow_softmax_topkpoint_topmask_fuse_8192/pythonfile/"
        # cp到checkpoints下
    ...
    if args.cuda:
        io.cprint('Using GPU')
        if args.manual_seed is not None:
            # 这个函数用于设置当前 CUDA 设备的随机种子。
            torch.cuda.manual_seed(args.manual_seed)
            # 这个函数用于设置所有可见的 CUDA 设备的随机种子。
            torch.cuda.manual_seed_all(args.manual_seed)
    else:
        io.cprint('Using CPU')
    io.cprint(str(args))

    # 这行代码的意图是从 args 对象中获取键为 'correlation_loss' 的值。
    # 如果 'correlation_loss' 不存在于 args 中，则返回默认值 False。
...
def xyz_restore(xyz_in_all, relax_proportion_all):
    B = xyz_in_all.shape[0]  # xyz_in_all为32*8192*3
    result = []
    for i in range(B):
        xyz_in = xyz_in_all[i, :]  # xyz_in 为8192*3
        relax_proportion = relax_proportion_all[i] #relax_proportion_all为32*6
        len_x, len_y, len_z, x_min, y_min, z_min = relax_proportion  # 1*6赋给他们
        x = xyz_in[:, 0].reshape(-1, 1)  # 8192*1 x坐标
        y = xyz_in[:, 1].reshape(-1, 1)  # 8192*1 y坐标
        z = xyz_in[:, 2].reshape(-1, 1)  # 8192*1 z坐标
        new_x = (x * (len_z / len_x) + 0.5) * len_x + x_min
        new_y = (y * (len_z / len_y) + 0.5) * len_y + y_min
        new_z = (z + 0.5) * len_z + z_min
        temp = torch.cat((new_x, new_y, new_z), axis=1).reshape(-1, 3)  # 水平上拼接，重新变成8192*3，现有维度上拼接，维度不改变
        result.append(temp)  #
    result = torch.stack(result)    # 变成tensor，新的维度上拼接，维度发生变化 最后result变成32*8192*3
    return result
...
def train_one_epoch(model, optimizer, epoch, args):
    ...
    for index, data in loop:
        mask_point1, mask_color1, mask_point2, mask_color2, mask_gt1, mask_gt2, nor_gt_pc, gt_pc, relax_ratio, _ = data
        # 将数据也转移到gpu上
        ...
        # record network acc # 精度通过两片点云之间的位移量的阈值决定
        acc += Miccai_absolute_strength_evaluate_result(xyz_restore(l_pred_xyz[0], relax_ratio), gt_pc, mask_gt1)
        normalized_acc += Miccai_absolute_strength_evaluate_result(l_pred_xyz[0], nor_gt_pc, mask_gt1)[-1] # 精度的最后一项，平均精度
        mask_acc += Mask_evaluate_result(torch.sigmoid(l_pred_mask1[0]), mask_gt1)
        # 更新信息
        loop.set_description(f'Epoch [{epoch}/{args.epochs}]')
        loop.set_postfix(loss=loss.item())
    acc = acc / (index + 1) * 100  # acc中为一个tensor张量，代表了不同容忍误差的精度cacc ，最后一项为平均值/100
    mask_acc = mask_acc / (index + 1) * 100  # 为一个tensor列表，代表了不同截断值的，mask的预测精度macc
    precise_mask_acc = precise_mask_acc / (index + 1) * 100  # 为0，因为precise_mask_acc=0
    normalized_acc = normalized_acc / (index + 1) * 100  # 为单个值
    return train_loss, [acc, mask_acc, precise_mask_acc, normalized_acc], [gt_loss_sum, mask_loss_sum, smooth_loss_sum,
                                                                       corr_loss_sum]

# 不同截断值下的成功率，最后一个为00.9
...
def test_one_epoch(model, args):
    ...
    with torch.no_grad():
        for index, data in tqdm(enumerate(args.test_loader), total=len(args.test_loader)):
            ...
            # record network acc  # 为什么要用第一个，是因为这是上采样的最后结果
            test_acc += Miccai_absolute_strength_evaluate_result(xyz_restore(l_pred_xyz[0], relax_ratio),
                                                                 gt_pc, mask_gt1)
            normalized_acc += Miccai_absolute_strength_evaluate_result(l_pred_xyz[0], nor_gt_pc, mask_gt1)[-1]
            test_mask_acc += Mask_evaluate_result(torch.sigmoid(l_pred_mask1[0]), mask_gt1)  # 预测的重叠比率与真值的比较
    normalized_acc = normalized_acc / (index + 1) * 100

    # 0.00001 除以总的batch，然后用%表示
...
def train(starting_epoch, model, optimizer, scheduler, io, writer, args):
    ...
    if args.dataset == 'normal':
        ...
        args.test_loader = DataLoader(
            ...
            shuffle=True, drop_last=True)
    best_acc = 1e-5
    # out_info 对象用来保存训练和测试过程中的各种信息，包括损失、准确率等。
    # util.py文件中
    out_info = CfgNode()
    for epoch\
            in range(starting_epoch, args.epochs):
        # Train
        ...
        # writer_info 函数将 out_info 的信息写入日志和可视化工具中，用于记录训练和测试的结果。
        ...
        # 当测试集精度高于best_acc = 1e-5时，则保存当前文件
        if out_info.test_acc[0][0] >= best_acc:
            best_acc = out_info.test_acc[0][0]

            # ！！！！！！！保存文件，最优权重
            # .module 常用于多 GPU 或者分布式训练中，用来访问模型的主体部分。
...
def main(args, io, writer):
    ...
    if args.resume is not None:
        ...
        # 从一个已经训练好的权重文件中读取epoch的数值，以继续以此进行训练
        ...
        starting_epoch = 0
    # 一个批次的输入数据（inputs）会被 nn.DataParallel
    # 自动分割成若干子集，子集的数量等于可用的 GPU 数量。
