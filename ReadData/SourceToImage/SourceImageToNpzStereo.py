...
def SourceToImageSingle(file_paths):
    target_path = "/big_data/szm/cwz_stereomis_new0610"
    for image_path in file_paths:

        # file_num = str(image_path[0].split(".")[-1])
        file_num = str(image_path.split("/")[-1].split(".")[0])  # .split("0")[-1]).rjust(4, '0')
        depth_path = os.path.join(os.path.dirname(os.path.dirname(image_path)), "depth/", f"show_dep0{file_num}"
                                   + ".png")
        store_path = os.path.join(target_path, file_num + ".npz")
        if os.path.exists(image_path) and os.path.exists(depth_path) and not os.path.exists(store_path):
            intrinsics_path = os.path.join(os.path.dirname(os.path.dirname(image_path)), "calib.txt")
            # 内参读取
...
if __name__ == "__main__":
    all_split_num = 10
    pool_num = 5
    root_path = r"/big_data/szm/cwz_stereomis_picture"
    # root_path = r"/big_data/szm/Cache_MICCAI_Hamlyn/HamlynSource"
    image01_paths = []
    # for folder_path in glob.glob(os.path.join(root_path, "jieyu_stereomis")):
    #     # 放彩色图
    image01_paths.extend(glob.glob(os.path.join(root_path, "output_frames", "output_frames_stereo", "*.jpg")))
    all_len = len(image01_paths)
    temp_len = all_len // all_split_num
    file_group_list = []
    last_number = 0
    for i in range(700, all_len, temp_len):
        if i != 0:
