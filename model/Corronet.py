...
class Encoder(pl.LightningModule):
    def __init__(self, enc_emb_dim=128, enc_glb_dim=1024, k_nn=20):
        ...
        self.conv4 = nn.Sequential(nn.Conv2d(128 * 2, 256, kernel_size=1, bias=False), self.bn4,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv5 = nn.Sequential(nn.Conv1d(512, enc_glb_dim // 2, kernel_size=1, bias=False), self.bn5,
                                   nn.LeakyReLU(negative_slope=0.2))
...
class LitCorrNet3D(pl.LightningModule):
    ...
    def step(self, batch, batch_idx):
        ...
        self.train_loss_last_step.append(
            loss.item())  # if use loss, then gpu leak in the 1st epoch for pytorch-lightning
        return loss, logs
