...
class Human_SelfSupervise_mask:
    ...
    def post_losscal_hamlyn_mask(self, l_xyz1, l_new_xyz, xyz2, color1, color2, l_mask1, l_mask2, mask_pseudo1,
        ...
        if single_mask1.sum() / single_mask1.shape[0] < 0.5 or single_mask2.sum() / single_mask2.shape[0] < 0.5:
            if need_fpfh:
                loss = nn_loss_sum * 2 + smooth_gradient_loss + gt_pseudo_loss * 3
                print("single_mask1.sum() / single_mask1.shape[0] < 0.5!!!!!!!!!!!!!!!!!!!!!!")
            else:
                loss = nn_loss_sum * 2 + smooth_gradient_loss
                print("single_mask1.sum() / single_mask1.shape[0] < 0.5!!!!!!!!!!!!!!!!!!!!!!else")
            # loss = nn_loss_sum
            ...
            if need_fpfh:
                print("single_mask1.sum() / single_mask1.shape[0] < 0.5？？？？？？？？？？？？？？？？？")
                loss = nn_true_loss_sum + nn_loss_sum + smooth_gradient_loss + gt_pseudo_loss * 3
            else:
...
def SH_PostTrainInit(type_name):
    ...
    os.system(
        'cp -r {} checkpoints/'.format(file_name) + args.exp_name + '/' + type_name + '/1{}.txt'.format(file_name))
    # -----------------------------------------------------------------------------
    # set random seed
    # -----------------------------------------------------------------------------
...
def post_train_main(model, args, io, sample_num, data, loss_fn):
    ...
    for epoch in range(0, args.pt_epoch):
        ...
        if epoch == 0:
            # 第一轮，还未训练，用的有监督模型的结果精度
            test_acc1 = Miccai_absolute_strength_evaluate_result(l_pred_xyz[0], mask_gt_pc, mask_gt1) * 100
            mask_acc1 = Mask_evaluate_result(torch.sigmoid(l_pred_mask1[0]), mask_gt1)
            precise_mask_acc1 = Miccai_absolute_strength_evaluate_result(pcd1=l_pred_xyz[0], pcd2=mask_gt_pc, mask=(
                    torch.sigmoid(l_pred_mask1[0]) > 0.9).squeeze(-1)) * 100
            best_result1 = [test_acc1.cpu(), mask_acc1.cpu(), precise_mask_acc1.cpu()]
            string = "supervise test_acc is {}, mask_acc is {}, precise_pred_mask_acc is {}".format(best_result1[0],
                                                                                                    best_result1[1],
                                                                                                    best_result1[2])
            io.cprint(string)
    ...
    if gt_pseudo_loss <= 3.5:

        # 每个样本循环完毕后测精度
        string = "unsupervised test_acc is {}, mask_acc is {}, precise_pred_mask_acc is {}".format(best_result[0],
                                                                                                   best_result[1],
                                                                                                   best_result[2])
        io.cprint(string)
        # args.best_result = best_result

        save_path = 'checkpoints/test_on_onesample_7.0_0.40/test_result/post_train_sample_num_{}_best.npz'.format(sample_num)
        np.savez(save_path,
                 points2=points2.cpu(),
                 points1=points1.cpu(),
                 colors1=colors1.cpu(),
...
if __name__ == "__main__":
    # args_g, io_g = SH_PostTrainInit("Post_Train_Hamlyn_no_rotation_75_datiao_814")\
    args_g, io_g = SH_PostTrainInit("Post_Train_Hamlyn_no_rotation_85_datiao_qiuqiu_noise_0516_7.0_0.40")
    ...
    #     root="/big_data/szm/H8amlyn_8192_Mask_3332_new_mutual_85/test1",
    #     fpfh_path="/big_data/szm/H8amlyn_8192_Mask_3332_new_mutual_85/fpft_file1",
    #     sample_num=sample_num, normalize=False, continuous=True),
    #     batch_size=1)
    test_hamlyn_loader = DataLoader(HamlynArtificialFPFH(
        root="/home/szm/Paconv_730/checkpoints/test_on_onesample_7.0_0.40",
        fpfh_path="/home/szm/Paconv_730/checkpoints/test_on_onesample_7.0_0.40/fpfh",
        sample_num=sample_num, normalize=False, continuous=True),
        batch_size=1)
    # test_hamlyn_loader = DataLoader(HamlynArtificialFPFH(
    #     root="/big_data/szm/H8amlyn_8192_Mask_3332_new_mutual_85noise/test_3.0_0.2",
    #     fpfh_path="/big_data/szm/H8amlyn_8192_Mask_3332_new_mutual_85noise/fpft_file_3.0_0.2",
    #     sample_num=sample_num, normalize=False, continuous=True),
    #     batch_size=1)
    ...
    for index, data in loop:
        save_path = 'checkpoints/test_on_onesample_7.0_0.40/test_result/post_train_sample_num_{}_best.npz'.format(index)
